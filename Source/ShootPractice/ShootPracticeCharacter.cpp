...
void AShootPracticeCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// Bind fire event
	PlayerInputComponent->BindAction("Fire", IE_Pressed, this, &AShootPracticeCharacter::OnFire);

	// Enable touchscreen input
	EnableTouchscreenMovement(PlayerInputComponent);

	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &AShootPracticeCharacter::OnResetVR);
	...
	if (ProjectileClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<AShootPracticeProjectile>(ProjectileClass, SpawnLocation, SpawnRotation,spawnParam);
			}
			else
			{
				...
				World->SpawnActor<AShootPracticeProjectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
			}
			//ºó×øÁ¦
			float recoilForce = 1.0f;
			float randNum = FMath::SRand();
			TurnAtRate(recoilForce * (randNum - 0.5));
			randNum = FMath::SRand();
			LookUpAtRate(-1*recoilForce * randNum);
		}
	}
...
void AShootPracticeCharacter::OnResetVR()
{
